...
samplefolder <- file.path("/home/jmbadia/Desktop","mzMLClaria")
...
result <- identifySpectra(UNKdata = UNK, stick2Adducts=F, cosSimTh=0.8, massErrorAllowed=10, samePrecMass=F, candidatesNumber=20, cl=2L, minCommonMassNum=2, topMassesNum = 5)

#FERHO F i F i després a l'excel q es vegi els T T, per veure si en podem treure algo de la resta de considerats
#saveRDS(result, file="esborraResult2.rds")

result_ALLDB <- readRDS(file="result_Clairo_ALLDB.rds")
result_stck2Addct <- readRDS(file="Result_Clairo_onlyStick2Adducts.rds")

result <- result_stck2Addct
#.------------------------------
# 5. POST-ID TOOLS ------------------------------
...
#plt everything to a pdf file 
for(idSample in unique(summary_result$file)){
  print(idSample)
  pdf(paste0("allDB_",idSample,".pdf"), width = 16 , height = 10, title = idSample)
  for(idRow in which(summary_result$file==idSample)){
    print(plotSpectra(unk=as.integer(summary_result[idRow,"UNKidSpectra"]), ref=as.integer(summary_result[idRow,"REFidSpectra"]), result))
  }
  dev.off()
}
...
saveWorkbook(wb, "Clairo_stck2Addct.xlsx", overwrite = TRUE)



#prepare pizza
REFidspectra_involved <-  which(DB$list_fragments$ID_spectra %in% result_stck2Addct$REFidSpectra)
...
pizza <- list(metadata=result_stck2Addct, UNKfragments=UNKfragments, REFfragments=REFfragments) 
saveRDS(pizza, "lot_Clairo_stck2Addct.rds")
